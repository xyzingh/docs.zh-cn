...
从 .NET 5.0 开始，某些 API 标记为已过时。 此中断性变更特定于标记为已过时并带有自定义诊断 ID 的 API。 取消默认过时诊断 ID（对于 C# 编译器，即为 [CS0618](../../../../docs/csharp/language-reference/compiler-messages/cs0618.md)），不会取消使用这些 API 时编译器生成的警告。
